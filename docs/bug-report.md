...
### 修复与定向回归追加记录（2026-09-18，BUG-TASK-001）

> 上面的“待修复”、原始缺陷描述、实际结果、代码关联及5张修复前截图为发现阶段记录，完整保留。以下只追加本次权限修复与验证，不否认已登录非成员曾能跨组读取、创建、修改和删除任务的历史。
...
**根因与修复方式：** 登录认证之后缺少任务资源级授权。修复复用现有 `getGroupAccess()`，新增小型 `getTaskAccess()`：先按task id只读查询任务的真实 `group_id`，再校验当前用户是否为该组owner或成员。指定group的GET和POST在读写前校验目标组成员；PUT和DELETE依据任务真实归属授权，不读取客户端伪造的 `group_id`；GET不传 `group_id` 时改为只返回当前用户创建或加入小组的任务。管理员继续使用原有独立接口。本轮没有新增普通任务内容编辑功能，也没有修改纯空格、失败HTTP 200或非法group id的既有处理逻辑，BUG-TASK-002/003/004保持原状态。

**修复后权限模型：**

| 操作 | 匿名 | 非成员 | 本组普通成员 | 组长 | 管理员 |
| --- | --- | --- | --- | --- | --- |
| 指定小组任务列表 | 401 | 403 | 200 | 200 | 使用独立管理员列表 |
| 普通任务创建 | 401 | 403 | 200 | 200 | 普通接口不提供管理员例外 |
| 普通任务状态修改 | 401 | 403 | 200 | 200 | 使用独立管理员编辑 |
| 普通任务删除 | 401 | 403 | 200 | 200 | 使用独立管理员删除 |
| 不带group筛选的任务列表 | 401 | 仅返回本人授权小组 | 仅返回本人授权小组 | 仅返回本人授权小组 | 使用独立管理员列表 |

删除继续维持既有产品语义：任务板对本组成员显示删除按钮，历史TASK-005也确认普通成员可创建并删除任务，因此本轮允许组长和本组成员删除本组任务，没有收紧成仅组长删除。

**真实HTTP与SQL验证结果：** 使用独立临时owner、member、outsider以及两个临时小组。匿名读取和创建均返回401；非成员指定组读取、创建、修改状态和删除均返回403。非成员在PUT请求中同时提交伪造的另一个 `group_id`、内容和合法状态，接口仍根据task真实归属返回403，SQL确认原内容、状态、group归属整行不变。普通接口不存在内容编辑语义，因此没有虚构“合法成员修改内容”用例；管理员独立内容编辑接口完成200回归。

合法成员读取和创建均返回200，创建记录的group与内容经SQL确认正确；状态从“待处理”更新为“进行中”后接口和SQL一致；本组成员删除返回200且SQL剩余为0。不带 `group_id` 的列表包含outsider自有小组任务，但不包含目标组任务。管理员全站任务列表、内容/状态编辑和删除分别返回200/200/200，既有后台能力保持正常。

**副作用、回归与清理：** 匿名及非成员创建前后目标内容记录均为0；非成员修改和删除前后目标任务整行一致。`demo_user` 真实登录后，group31详情和任务列表均返回200，任务数量与验证前基线一致。测试结束后临时用户、小组、任务、成员关系均为0；A/B/C/admin账号字段、group31详情、成员与任务逐行对比均未改变。README和原功能测试统计未修改。

**修复后证据：**

![BUG-TASK-001：匿名读取和创建返回401且无数据库新增](./screenshots/FIX-TASK-001-anonymous-401.png)

![BUG-TASK-001：非成员读取返回403且无筛选列表不再泄露跨组任务](./screenshots/FIX-TASK-001-nonmember-read-403.png)

![BUG-TASK-001：非成员创建、内容写入尝试和状态修改返回403且无副作用](./screenshots/FIX-TASK-001-nonmember-write-403.png)

![BUG-TASK-001：非成员删除返回403且目标任务整行保持不变](./screenshots/FIX-TASK-001-nonmember-delete-403.png)

![BUG-TASK-001：合法成员与管理员任务流程回归通过](./screenshots/FIX-TASK-001-valid-member-flow.png)

![BUG-TASK-001：临时数据清零且group31基线未改变](./screenshots/FIX-TASK-001-cleanup.png)
